...
open class BasicAudioController: NSObject, AVAudioPlayerDelegate {
    ...
    open func configureAudioCell(_ cell: CustomVoiceChatView, message: MessageType) {
        if playingMessage?.messageId == message.messageId, let player = audioPlayer {
            playingCell = cell
            cell.durationLabel.text = cell.getDurationFormat(duration: Float(player.currentTime))
        }
    }
    ...
    open func playSound(for message: MessageType, in audioCell: CustomVoiceChatView) {
        ...
        case .audio(let item):
            ...
            audioPlayer?.play()
            state = .playing
            audioCell.playImageView.image = UIImage(systemName: "pause.fill")  // show pause button on audio cell
            startProgressTimer()
        default:
    ...
    open func pauseSound(for message: MessageType, in audioCell: CustomVoiceChatView) {
        audioPlayer?.pause()
        state = .pause
        audioCell.playImageView.image = UIImage(systemName: "play.fill") // show play button on audio cell
    ...
    open func stopAnyOngoingPlaying() {
        ...
        if let cell = playingCell {
            cell.playImageView.image = UIImage(systemName: "play.fill") // show play button on audio cell
            cell.durationLabel.text = cell.getDurationFormat(duration: Float(player.duration))
        }
        progressTimer?.invalidate()
        progressTimer = nil
        audioPlayer = nil
    ...
    open func resumeSound() {
        ...
        state = .playing
        startProgressTimer()
        cell.playImageView.image = UIImage(systemName: "pause.fill") // show pause button on audio cell
    }
